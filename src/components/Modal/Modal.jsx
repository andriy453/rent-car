...
function Modal({onClose,arr}) {
  const { make, model, rentalPrice, img,id,address,rentalConditions,type,mileage,accessories,year,fuelConsumption,engineSize,description,functionalities,photoLink} = arr
      useEffect(()=>{
...
return createPortal(
  <Backdrop onClick={onOverlayClose} >
    <ModalContent>
            ...
            <CarTitle $size={true}>{make} {<CarModel>{model}</CarModel>}, {year}</CarTitle>
            ...
            <ModalRentalList><RentalItem> Minimum age :<span>{rentalConditions.slice(13, 16)}</span></RentalItem>  <RentalItem> Valid driver’s license</RentalItem> <RentalItem> {rentalConditions.slice(38, rentalConditions.length)}</RentalItem>  <RentalItem>Mileage:<span>{mileage.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</span></RentalItem><RentalItem> Price:<span>{rentalPrice.slice(1)}$</span></RentalItem>  </ModalRentalList>
